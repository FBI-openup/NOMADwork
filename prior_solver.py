...
# 基础权重 | Base weights (proportional to scoring weights 0.4/0.2/0.3/0.1, scaled ×100)
W_TRAFFIC = 40.0
W_DELAY   = 20.0
W_DIST    = 30.0
W_LAND    = 10.0
...
INERTIA_BONUS = 2.0          # 上秒在同一落地 / Previous second inertia
ETA_BONUS_B   = 3.0          # 下一秒 B 窗口 / Next second B window bonus
ETA_BONUS_HB  = 1.5          # 下一秒 B/2 窗口 / Next second B/2 window bonus
COMPLETE_BONUS_ALPHA = 0.05  # 完成度奖励系数 / Completion reward coefficient

# 【已禁用】饥饿保护 | 【DISABLED】Starvation Protection
# 原设置：STARVE_THRESH = 3, STARVE_BOOST = 2.0
# 新设置：完全关闭 → 只按优先级分配，无公平性补偿
# Original: STARVE_THRESH = 3, STARVE_BOOST = 2.0
# New: Disabled entirely → allocate purely by priority, no fairness compensation
STARVE_THRESH = 10**9        # 永远不会触发 / Never reached
STARVE_BOOST  = 0.0          # 不加成 / No bonus applied

# 其他超参数 | Other Hyper-parameters
SWITCH_EPS_FACTOR = 1.2      # 切换阈值系数 / Landing switch penalty factor (>1 = more conservative)
BETA_HOP = 0.1               # 曼哈顿距离衰减系数 / Manhattan distance decay (2^{-beta*h})
TOP_K = 99                    # 保留所有候选 / Keep all candidates, let global optimization decide
